...
function getPolynom(...args) {
  return (x) => [...args].reverse().reduce((p, c, i) => p + c * x ** i);
}
 ...
 */
function memoize(func, cached = func()) {
  return () => cached;
}
...
function retry(func, attempts) {
  return (...a) => {
    try {
      ...
      return retry(func, attempts - 1)(a);
    }
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    const string = `${func.name}(${JSON.stringify(args).slice(1, -1)})`;
